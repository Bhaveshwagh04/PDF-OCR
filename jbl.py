...
def generate_chart(df, chart_type, title=None, x_axis_label=None, y_axis_label=None, color_scale=None, bin_size=None):
    ...
    if chart_type == 'bar':
        ...
        fig = go.Figure(go.Waterfall(
            x=df.columns[0],
            y=df.columns[1],
            name=title if title else f"Waterfall of {df.columns[0]} vs. {df.columns[1]}"
        ))
        fig.update_layout(title=title if title else f"Waterfall of {df.columns[0]} vs. {df.columns[1]}",
                           xaxis_title=x_axis_label if x_axis_label else df.columns[0],
                           yaxis_title=y_axis_label if y_axis_label else df.columns[1])
